...
export class Sunrise {
    constructor(document, skyProportion = 0.5) {
    ...
    wheelPos2sunPos() {
        ...
        if (this.curPos < nnPx) {
            ...
            const srPx = (this.myWidth / 24 / 60) *
                (sunrise.getHours() * 60 + sunrise.getMinutes());
            //y = -(x-srPx)/(nnPx-srPx)*hr + hr
            return {
                clientX: this.curPos,
                clientY: (-(this.curPos - srPx) / (nnPx - srPx)) *
                    this.myHeight *
                    this.skyProportion +
            ...
            const ssPx = (this.myWidth / 24 / 60) *
                (sunset.getHours() * 60 + sunset.getMinutes());
            //y = (x-ssPx)/(nnPx-ssPx)*hr + hr
            return {
                clientX: this.curPos,
                clientY: (-(this.curPos - ssPx) / (nnPx - ssPx)) *
                    this.myHeight *
                    this.skyProportion +
                    this.myHeight * this.skyProportion,
            };
    ...
    moveSun(e) {
        ...
        if (sun) {
            ...
            CSS_PREFIXES.forEach((prefix) => {
                sun.style.background = `${prefix}(${this.mouse.x}px ${this.mouse.y}px, ${build_path(sun_path_data)}`;
            });
        }
        const sunDay = document.getElementById("sunDay");
        if (sunDay) {
            const sun_date_path_data = [
            ...
            CSS_PREFIXES.forEach((prefix) => {
                sunDay.style.background = `${prefix}(${this.mouse.x}px ${this.mouse.y}px, ${build_path(sun_date_path_data)}`;
            });
        }
        const sunSet = document.getElementById("sunSet");
        if (sunSet) {
            const sun_set_path_data = [
            ...
            CSS_PREFIXES.forEach((prefix) => {
                sunSet.style.background = `${prefix}(${this.mouse.x}px ${this.mouse.y}px, ${build_path(sun_set_path_data)}`;
            });
        }
        ...
        !sky
            ...
            : (sky.style.opacity =
                "" +
                    Math.min(1 - this.mouse.y / (this.myHeight * this.skyProportion * 2), 0.99));
        !sunSet
            ? null
    ...
    darknessOverlayOpacity() {
        return Math.min((this.mouse.y - this.myHeight * this.skyProportion) /
            (this.myHeight * this.skyProportion), 1);
    }
    darknessOverlaySkyOpacity() {
        return Math.min((this.mouse.y - (this.myHeight * 7) / 10) /
    ...
    waterReflectionMiddleOpacity() {
        ...
        if (that.sunBelowHorizon()) {
            return ((that.myHeight - that.mouse.y) /
